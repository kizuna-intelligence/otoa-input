...
pub struct SettingsExtension {
    ...
    pub build_sections: Option<
        Arc<dyn Fn(Settings, ui::UiState, Sender<ControllerCommand>) -> floem::AnyView + Send + Sync>,
    >,
    /// 足す塊を、公開版の枠より**前**に置くか。既定は後ろ。
    ///
    /// 面の主役が配布側にあるときは前へ置く。公開版の細かい調整が上に
    /// 大きく載っていると、利用者はまずそれを読まされる。
...
設定はトレイアイコンの「設定」から変更する。起動中にもう一度 otoa-input を
起動すると、動いている方へ「設定画面を開け」と合図して終了する
（メニューバーが混んでいてトレイアイコンに手が届かないとき用）。
