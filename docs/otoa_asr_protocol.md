...
- 端末 VAD が無音を検知しても、サーバーから `<end>` を受けるまでは送る
  （先に止めるとサーバーが発話終了を判定できない）
- `<end>` を受けた後は送信を止め、次の端末 VAD の SpeechStarted で直前 500 ms
  のプリロールから再開する。終話済みの無音を新しい空発話として送らないためである
...
端末の VAD が「発話が終わった」と伝える手段は**用意しない**。端末の VAD が
決めてよいのは「話し始め」だけで、発話が終わったかどうかは認識器が判断する。
端末が終わりを伝えると、その判断を端末が肩代わりすることになる。
...
```json
{
  "notice_code": "gate_blocked",
  "notice_message": "登録した声と一致しませんでした。設定から登録し直すか、マイクを確認してください。"
}
```

- `notice_code` は安定した機械可読の識別子、`notice_message` は利用者に表示する文言である
- `tokens` / `finished` / `error_*` と**同じフレームに混ぜない**。独立した Text フレームとして送る
- 通知を受信しても接続は閉じず、文字起こし・クリップボード出力にも混ぜない

---

## 8. クライアントの前提（サーバー実装者向けの注意）

otoa-input は次のように振る舞う。サーバーはこれを前提にしてよい。

1. **発話の開始は端末側の VAD で判断する。** 検知した時点で接続し、
   直前 500 ms 分の音声（プリロール）を先に送ってから実時間送出に移る
2. **発話の終了は判断しない。** 端末 VAD が無音を検知しても**サーバーへは何も
   送らず**、`<end>` を受けるまで音声を送り続ける(無音も送る)。表示だけは
   端末の都合で先に「応答待ち」へ変えてよい
3. `<end>` を受けたら、その発話を確定してクリップボードへ貼り付け、次の発話まで
   音声送信を止める
4. 最後の `<end>` から一定時間（既定 120 秒）で空フレームを送り接続を閉じる
...
- `notice_code` / `notice_message` を知らない公開版 v0.2.0 等は、この通知フレームを
  未知フィールドとして無視する。通知の追加だけで既存クライアントの接続を壊さない
